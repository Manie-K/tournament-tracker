...
public class MatchRestController implements MatchController
{
    ...
    public GetMatchesResponse getMatches() {
        return factory.matchesToResponse().apply(service.findAll());
    }

    @Override
    public GetMatchesResponse getUserMatches(UUID userId) {
        return factory.matchesToResponse().apply(service.findAllByUser(userId));
    }

    @Override
    public GetMatchesResponse getTournamentMatches(UUID tournamentId) {
        return factory.matchesToResponse().apply(service.findAllByTournament(tournamentId));
    }

    @Override
    public GetMatchResponse getMatch(UUID id) {
        return factory.matchToResponse().apply(service.find(id).orElseThrow(NotFoundException::new));
    }

    @Override
    public void putMatch(UUID id, PutMatchRequest request) {
        service.create(factory.requestToMatch().apply(id, request));
    }

    @Override
    public void patchMatch(UUID id, PatchMatchRequest request) {
        factory.updateMatch().apply(service.find(id).orElseThrow(NotFoundException::new), request);
    }
